...
protocol FavoritesService {
    func loadFavorites(id: Int, completion: @escaping (Result<[Concert], ConcertErrors>) -> Void)
...
class RestFavoritesService: FavoritesService {
    let listFavoritesUrl: URL = {
        guard let url = URL(string: "http://localhost:8080/concerts/favorites") else {
    ...
    func loadFavorites(id: Int, completion: @escaping (Result<[Concert], ConcertErrors>) -> Void) {
        ...
        let request = jsonRequest(token: token, id: id, url: listFavoritesUrl)
        ...
        AF.request(request).validate(statusCode: 200..<300)
            ...
            case .failure:
                completion(.failure(.errorGetFavoriteConcerts))
            }
            }
    }
